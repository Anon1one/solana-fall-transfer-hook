...
use helpers::{
    ...
    build_transfer_via_token_mover_ix,
};
...
#[test]
fn test_transfer_via_token_mover() {
    let (mut svm, payer, program_id) = setup();
    let mint = Keypair::new();
    ...
    let transfer_ix = build_transfer_via_token_mover_ix(
    ...
    let res = svm.send_transaction(tx);
    assert!(res.is_ok(), "Transfer via token-mover failed: {:?}", res.err());
}

#[test]
fn test_transfer_via_token_mover_rate_limit_exceeded() {
    let (mut svm, payer, program_id) = setup();
    let mint = Keypair::new();
    ...
    let ix1 = build_transfer_via_token_mover_ix(
    ...
    let tx = VersionedTransaction::try_new(VersionedMessage::Legacy(msg), &[&payer]).unwrap();
    let res = svm.send_transaction(tx);
    assert!(res.is_ok(), "First transfer via token-mover should succeed: {:?}", res.err());

    let ix2 = build_transfer_via_token_mover_ix(
    ...
    assert!(res.is_err(), "Second transfer via token-mover should fail");

    let err = format!("{:?}", res.unwrap_err());
    assert!(err.contains("0x1771") || err.contains("6001"), "Expected rate limit exceeded error, got: {}", err);
}
